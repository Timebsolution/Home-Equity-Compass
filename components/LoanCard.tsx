                    ...
                    <div className={`border rounded-lg ${theme==='light'?'border-gray-200 bg-gray-50':'border-gray-700 bg-black/20'}`}>
                        ...
                        <div className="p-2 space-y-1">
                            {!scenario.isRentOnly && !scenario.isInvestmentOnly ? (
                                ...
                                <>
                                    <BreakdownRow label="Investment Growth" value={`+${formatCurrency(rRealInvGain)}`} colorClass={rRealInvGain >= 0 ? "text-emerald-500 font-bold" : "text-red-500 font-bold"} icon={<TrendingUp size={10} />} tooltip="Profit from portfolio (Interest Only)" />
                                </>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="p-2 space-y-1">
                            {!scenario.isRentOnly ? (
                                <>
                                    ...
                                    {/* HIDE Investment Contributions for Buy/Refi to avoid double counting */}
      ...
      {!isExpanded && (
        ...
        >
            ...
            <div className="pt-2 border-t border-dashed border-gray-500/20 space-y-2">
                <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold uppercase ${theme === 'light' ? 'text-gray-400' : 'text-gray-500'}`}>{mainMetricLabel}:</span>
                    <span className={`text-sm font-extrabold tracking-tight ${isWinner ? (theme==='light' ? 'text-emerald-600' : 'text-emerald-400') : (theme === 'light' ? 'text-gray-700' : 'text-gray-200')}`}>
                        {formatCurrency(mainMetricValue)}
                    </span>
                </div>
                {!scenario.isRentOnly && !scenario.isInvestmentOnly && (
